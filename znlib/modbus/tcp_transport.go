...
*/
func (tt *tcpTransport) ReadRawData(len uint8) ([]byte, error) {
	// set an i/o deadline on the socket (read and write)
	tt.socket.SetDeadline(time.Now().Add(tt.timeout))

	var buf = make([]byte, len)
	n, err := io.ReadFull(tt.socket, buf)
	if n > 0 {
		return buf[:n], err
	}
...
/*
  ...
  描述: 将 data 写入当前链路
*/
func (tt *tcpTransport) WriteRawData(data []byte) error {
	// set an i/o deadline on the socket (read and write)
	err := tt.socket.SetDeadline(time.Now().Add(tt.timeout))
	if err != nil {
		return err
