...
// ReadRawData 2026-02-09 12:35:35 +dmzn
/*
 参数: len,待读取数据长度
 描述: 从当前链路读取 len 长度的数据
*/
func (mc *ModbusClient) ReadRawData(len uint8) ([]byte, error) {
	if mc.transport == nil {
		return nil, fmt.Errorf("modbus transport is nil")
	}

	return mc.transport.ReadRawData(len)
}

// WriteRawData 2026-02-09 12:36:23 +dmzn
/*
 参数: data,待写入数据
 描述: 将 data 写入当前链路
*/
func (mc *ModbusClient) WriteRawData(data []byte) (err error) {
	if mc.transport == nil {
		return fmt.Errorf("modbus transport is nil")
	}

	return mc.transport.WriteRawData(data)
}
